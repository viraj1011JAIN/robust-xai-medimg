...
class TriObjectiveLoss:
    ...
    def _maybe_expl_loss(self, x_clean: torch.Tensor, x_adv: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        # frequency + subsample gate (saves memory on 4GB GPUs)
        self._step += 1
        ...
        with torch.no_grad():
            cam_clean = self.gradcam.generate(x_clean.detach())  # [B,H,W] in [0,1]
            cam_adv   = self.gradcam.generate(x_adv.detach())    # [B,H,W] in [0,1]

        if not _HAS_SSIM:
            # fallback: simple L1 distance of heatmaps
            return F.l1_loss(cam_clean, cam_adv)

        # SSIM wants [B,1,H,W], range [0,1]
        ssim_val = ssim(
    ...
    def __call__(self, model: nn.Module, x: torch.Tensor, y: torch.Tensor):
        ...
        if self.attacker is not None:
            # attacker must do full-precision internally; returns [0,1]
